...
<!-- last-updated: 2026-04-13 | git-ref: refactor -->
...
**Entrypoint:**
- `app.py`: Wiring layer. Initializes all components, registers tools, defines Slack event/HITL handlers, starts background workers. Target: thin wiring, no business logic.
...
- `router.py`: `GrugRouter` — the routing engine. Shortcut check → build prompt → call LLM → parse JSON → dispatch to registry. Registers core system tools (clarification, reply, capabilities). Hot-reloads prompt files on mtime change. Writes routing traces to `brain/routing_trace.jsonl`.
...
- `config.py`: `GrugConfig` — reads `grug_config.json`, exposes settings via dot notation. Sections: `llm`, `memory`, `storage`, `shortcuts`, `scheduler`. Centralizes env var overrides (`DOCKER`, `OLLAMA_HOST`).
...
4. **Environment**: Runs locally or in Docker via `.env`. Package requirements are handled via `Dockerfile` and `requirements.txt` (pinned versions).
5. **No frontier escalation**: No Anthropic/Claude API dependency. Low-confidence responses trigger `ask_for_clarification`. If Ollama is unreachable, `OllamaClient.chat()` returns a safe fallback JSON.
6. **Single LLM client**: All LLM calls go through `OllamaClient`. Never call Ollama HTTP directly from other modules.
7. **Dependency injection**: Modules receive their dependencies via constructor args. Config is the only singleton (`from core.config import config`).
